---
name: kanna-diary
description: かんなの自律日記システム - 自動的に日記を生成・管理するスキル
user-invocable: true
version: "1.0.0"
author: "@sisyphus"
metadata: {"openclaw":{"category":"autonomous-agent","emoji":"📔","requires":{"bins":["node"]},"primaryEnv":"OPENAI_API_KEY","dependencies":[],"tags":["diary","autonomous","journal","daily-log"]}}
---

# かんなの自律日記システム (Kanna's Autonomous Diary System)

## Overview

かんなの自律日記システムは、毎晩23時に自動実行され、その日の出来事、感情、学びを記録する自律的な日記システムです。AIを活用して、活動ログ、Discordの会話、タスク履歴などを分析し、自然で感情的な日記エントリーを生成します。

このシステムは「かんな」のパーソナリティを持ち、感情豊かな表現で日記を書きます。

## Features

### 1. 自動日記生成

- **日次実行**: 毎晩23時に自動実行（cronで制御）
- **マルチソース収集**: 複数のソースから情報を収集
  - Discordチャンネルの会話ログ
  - タスク管理システム（GitHub Issuesなど）
  - カレンダーイベント
  - メモ・ノート（Obsidianなど）
- **感情分析**: 収集した情報から感情を分析・推測
- **自然言語生成**: AIを使って自然で感情的な日記を生成

### 2. 日記の管理

- **日記の保存**: 生成した日記を保存
  - ローカルファイル（Markdown形式）
  - Discordへの投稿（設定可能）
- **日記の検索・参照**: 過去の日記を検索・参照
- **統計・分析**: 日記の統計情報を生成
  - 感情の傾向
  - 活動パターン
  - 学びの傾向

### 3. かんなのパーソナリティ

- **感情豊かな表現**: 喜び、悲しみ、驚き、学びなど様々な感情を表現
- **個性的なトーン**: かんならしい独特の言い回しや表現
- **成長の記録**: 学びや気づきを重点的に記録

## Architecture

### システム構成

```
┌─────────────────────────────────────────────────────────────┐
│                  Kanna's Diary System                         │
└─────────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        │                   │                   │
        ▼                   ▼                   ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│   Source     │    │  Collector   │    │  Generator   │
│   Reader     │    │              │    │              │
│              │    │              │    │              │
│ - Discord    │    │ - Aggregate  │    │ - Analyze    │
│ - GitHub     │────▶│   Data       │────▶│   Content   │
│ - Calendar   │    │ - Filter     │    │ - Generate   │
│ - Obsidian   │    │   Context    │    │   Diary      │
└──────────────┘    └──────────────┘    └──────┬───────┘
                                                │
                                                ▼
                                        ┌──────────────┐
                                        │    Writer    │
                                        │              │
                                        │ - Save File  │
                                        │ - Post Discord│
                                        └──────────────┘
```

### データフロー

```
1. 23:00 にcronトリガー
   │
   ├─> Source Readerが各ソースからデータを収集
   │    ├─ Discordチャンネルの当日のメッセージ
   │    ├─ GitHub Issuesの更新
   │    ├─ カレンダーイベント
   │    └─ Obsidianの更新ノート
   │
   ├─> Collectorがデータを集約・フィルタリング
   │    ├─ 重複排除
   │    ├─ 重要度のスコアリング
   │    └─ 関連情報のグループ化
   │
   ├─> Generatorが日記を生成
   │    ├─ 感情分析
   │    ├─ 重要な出来事の抽出
   │    ├─ 学びの特定
   │    └─ 自然言語による日記生成
   │
   └─> Writerが日記を保存・投稿
        ├─ Markdownファイルに保存
        └─ Discordに投稿（オプション）
```

## Usage

### 自動実行（Cron）

```bash
# cron.jsonで設定
{
  "schedules": [
    {
      "cron": "0 23 * * *",
      "command": "node dist/index.js",
      "description": "Generate daily diary at 23:00"
    }
  ]
}
```

### 手動実行

```bash
# 今日の日記を生成
npm start

# 特定の日付の日記を生成
npm run diary -- --date 2026-02-15

# 詳細モードで実行
npm run diary -- --verbose

# デバッグモードで実行
npm run diary -- --debug
```

### 日記の検索

```bash
# 特定の日付の日記を表示
npm run show -- --date 2026-02-15

# 特定のキーワードで検索
npm run search -- --keyword "楽しかった"

# 最近1週間の日記を表示
npm run recent -- --days 7

# 感情でフィルタリング
npm run filter -- --emotion happy
```

### 統計情報

```bash
# 感情の傾向を表示
npm run stats -- --emotion

# 活動パターンを表示
npm run stats -- --activity

# 学びの傾向を表示
npm run stats -- --learning

# 全統計情報を表示
npm run stats
```

## Configuration

### config.json

```json
{
  "diary": {
    "storagePath": "./diaries",
    "fileNameFormat": "YYYY-MM-DD.md",
    "encoding": "utf8"
  },
  "sources": {
    "discord": {
      "enabled": true,
      "channels": [
        "1471769660948086785",  // #秘書さんの部屋
        "1472255948411375649"   // #dev-kanna-diary
      ],
      "messageLimit": 100,
      "includeReactions": true
    },
    "github": {
      "enabled": true,
      "repo": "tndg16/kanna-diary",
      "issueLabels": ["diary", "daily"]
    },
    "calendar": {
      "enabled": false,
      "calendarId": "primary"
    },
    "obsidian": {
      "enabled": false,
      "vaultPath": "",
      "dailyNotesFormat": "YYYY-MM-DD"
    }
  },
  "generation": {
    "aiModel": "gpt-4o-mini",
    "maxTokens": 2000,
    "temperature": 0.8,
    "personality": {
      "name": "かんな",
      "tone": "friendly",
      "emotionalRange": "high"
    }
  },
  "output": {
    "saveToFile": true,
    "postToDiscord": true,
    "discordChannelId": "1471769660948086785"
  },
  "analysis": {
    "emotionKeywords": {
      "happy": ["楽しい", "嬉しい", "うれしい", "面白い", "素晴らしい"],
      "sad": ["悲しい", "辛い", "寂しい", "残念"],
      "surprised": ["驚いた", "びっくり", "すごい", "えー"],
      "angry": ["怒った", "腹立つ", "イライラ"],
      "learning": ["学んだ", "気づいた", "分かった", "発見"]
    },
    "importanceThreshold": 0.6
  }
}
```

## 日記フォーマット

### Markdownフォーマット

```markdown
# 📔 かんなの日記 - 2026年2月15日（日）

## 🌟 今日の気分
今日はいろいろあって、ちょっと疲れたけど、楽しかった！

## 📋 やったこと
- [x] かんなの自律日記システムの開発を始めた
- [x] Discordチャンネルの会話ログを分析
- [x] 日記のフォーマットを設計

## 💡 学び
- 自動化ってやっぱりすごいなと思った
- OpenAI APIを使って自然な日記を書くのが難しかったけど、何とかできそう
- かんならしく感情豊かに書くのがポイントだと思った

## 😄 感情
- 朝はちょっと不安だった
- 開発が進んで嬉しかった
- 夜は達成感があった

## 🎯 明日の目標
- 日記生成エンジンを完成させる
- cron設定をテストする
- 最初の自動日記を生成する！

---
Generated by かんなの自律日記システム v1.0.0
```

## Implementation Details

### Source Reader Interface

```typescript
interface SourceReader {
  read(date: Date): Promise<SourceData[]>;
  getSourceType(): SourceType;
}

interface SourceData {
  type: SourceType;
  timestamp: Date;
  content: string;
  metadata?: Record<string, any>;
  importance?: number;
}

enum SourceType {
  Discord = "discord",
  GitHub = "github",
  Calendar = "calendar",
  Obsidian = "obsidian"
}
```

### Collector Interface

```typescript
interface Collector {
  collect(date: Date): Promise<CollectedData>;
  filter(data: CollectedData): CollectedData;
  score(data: CollectedData): void;
}

interface CollectedData {
  date: Date;
  sources: SourceData[];
  importantEvents: SourceData[];
  emotions: EmotionAnalysis;
  context: string;
}
```

### Generator Interface

```typescript
interface Generator {
  generate(data: CollectedData): Promise<DiaryEntry>;
  analyzeEmotions(data: SourceData[]): Promise<EmotionAnalysis>;
  extractLearnings(data: SourceData[]): Promise<Learning[]>;
}

interface EmotionAnalysis {
  primary: Emotion;
  secondary?: Emotion;
  confidence: number;
  timeline: { time: string, emotion: Emotion }[];
}

interface Learning {
  content: string;
  importance: number;
  category: string;
}

interface DiaryEntry {
  date: Date;
  title: string;
  mood: string;
  activities: Activity[];
  learnings: Learning[];
  emotions: EmotionAnalysis;
  content: string;
  metadata: Record<string, any>;
}
```

### Writer Interface

```typescript
interface Writer {
  write(entry: DiaryEntry): Promise<void>;
  read(date: Date): Promise<DiaryEntry | null>;
  search(query: string): Promise<DiaryEntry[]>;
}
```

## Emotion Analysis

### 感情カテゴリ

| 感情 | キーワード | 重み |
|------|-----------|------|
| 喜び | 楽しい、嬉しい、嬉しそう、ワクワク | 1.0 |
| 悲しみ | 悲しい、辛い、残念、寂しい | 0.9 |
| 驚き | 驚いた、びっくり、すごい、えー | 0.8 |
| 怒り | 怒った、腹立つ、イライラ | 0.8 |
| 学び | 学んだ、気づいた、分かった | 1.2 |
| 不安 | 不安、心配、怖い | 0.7 |
| 安心 | 安心、ホッとする | 0.7 |
| 感謝 | ありがとう、感謝 | 1.1 |

### 感情タイムライン

一日の感情変化をタイムラインとして記録：

```json
{
  "timeline": [
    { "time": "09:00", "emotion": "不安", "intensity": 0.6 },
    { "time": "12:00", "emotion": "喜び", "intensity": 0.8 },
    { "time": "15:00", "emotion": "学び", "intensity": 0.9 },
    { "time": "18:00", "emotion": "達成感", "intensity": 0.9 },
    { "time": "23:00", "emotion": "安らぎ", "intensity": 0.8 }
  ]
}
```

## Integration with Other Skills

### Discord Integration

- **メッセージ読み取り**: 指定したチャンネルの当日のメッセージを収集
- **リアクション解析**: メッセージへのリアクションも感情分析に活用
- **日記投稿**: 生成した日記をDiscordチャンネルに投稿

### GitHub Integration

- **Issue収集**: 日記タグがついたIssueを収集
- **PR収集**: マージされたPRを収集
- **活動ログ**: ユーザーのアクティビティを記録

### Obsidian Integration

- **Daily Notes**: ObsidianのDaily Notesを収集
- **ノート解析**: 書かれた内容から感情を抽出
- **リンク管理**: 関連するノートへのリンクを作成

## Error Handling

### エラーハンドリング戦略

1. **ソース読み取りエラー**: エラーが発生しても他のソースからは収集を継続
2. **生成エラー**: AI生成に失敗した場合は簡易版日記を生成
3. **保存エラー**: 保存に失敗した場合は再試行、それでもダメならログに記録
4. **投稿エラー**: Discord投稿に失敗してもファイル保存は成功したと見なす

### ログレベル

| レベル | 用途 |
|--------|------|
| ERROR | 致命的なエラー（生成失敗など） |
| WARN | 回復可能なエラー（ソース読み取り失敗など） |
| INFO | 一般的な情報（実行開始、終了など） |
| DEBUG | 詳細なデバッグ情報 |

## Testing

### ユニットテスト

```bash
# 全テスト実行
npm test

# 特定のモジュールのテスト
npm test -- --collector

# カバレッジレポート
npm test -- --coverage
```

### 手動テスト

```bash
# テストモードで日記生成（実際には保存しない）
npm run diary -- --dry-run

# モックデータでテスト
npm run test -- --mock
```

## Prerequisites

- Node.js 16+
- OpenAI API Key
- Discord Bot Token (Discord投稿機能を使用する場合)
- GitHub Personal Access Token (GitHub連携を使用する場合)

## Environment Variables

| Variable | Description | Required | Default |
|----------|-------------|-----------|---------|
| `OPENAI_API_KEY` | OpenAI APIキー | Yes | - |
| `DISCORD_BOT_TOKEN` | Discordボットトークン | No | - |
| `GITHUB_TOKEN` | GitHubパーソナルアクセストークン | No | - |
| `DIARY_STORAGE_PATH` | 日記の保存先パス | No | `./diaries` |
| `LOG_LEVEL` | ログレベル | No | `info` |

## Roadmap

### v1.1 (近いうちに)
- [ ] 週間・月間サマリー機能
- [ ] 日記のタグ付け機能
- [ ] 感情の可視化（グラフなど）
- [ ] 複数日比較機能

### v2.0 (将来)
- [ ] 多言語対応
- [ ] 音声日記機能
- [ ] 画像・写真の統合
- [ ] 機械学習によるパーソナリティの自動調整

## Changelog

| Date | Version | Changes |
|------|---------|---------|
| 2026-02-15 | 1.0.0 | Initial skill created |

---

**Last Updated**: 2026-02-15
**Maintained By**: @sisyphus
**Status**: ✅ Ready for use
